...
import useSkillDataImport from 'src/hooks/use-skill-data-import';
...
export function SkillDataImport() {
  const { mutate: uploadMutation } = useSkillDataImport();
  ...
  const validateFile = async (event) => {
    ...
    if (selectedFile) {
      ...
      if (fileName.startsWith('Resource_Skill')) {
  ...
  return (
    <div className="flex w-full justify-center gap-2  items-center mb-4 text-blue-600">
      <Box
        height={54}
        width={'100%'}
        border={0.5}
...
export default SkillDataImport;
