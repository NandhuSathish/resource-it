...
export default function BillabilitySummaryResourceState() {
  const [open, setOpen] = useState(null);
  const billabilitySummaryQuery = useBillabilitySummaryQueryStore((s) => s.billabilitySummaryQuery);
  const setStatus = useBillabilitySummaryQueryStore((s) => s.setStatus);
  useEffect(() => {
    setStatus(1);
  }, []);
  ...
  const currentLabel =
    SORT_OPTIONS.find((option) => option.value === billabilitySummaryQuery.status)?.label ||
    'Active';
    ...
    <div className="ml-2">
      ...
      >
        {SORT_OPTIONS.map((option) => (
          <MenuItem
            key={option.value}
            selected={option.value === billabilitySummaryQuery.status}
