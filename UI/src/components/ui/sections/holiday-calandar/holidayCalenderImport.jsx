...
import useHolidayUpload from 'src/hooks/use-import-holiday';
import useHolidayCalendar from 'src/hooks/use-holiday-calendar';
import { toast } from 'sonner';
import { Box } from '@mui/material';
import { LoadingButton } from '@mui/lab';

export function HolidayCalendarImport() {
  const { mutate: uploadMutation } = useHolidayUpload();
      const [boardedFile, setBoarded] = useState(null);
  ...
  const validateFile = async (event) => {
    ...
    if (selectedFile) {
      ...
      if (fileName.startsWith('INV_Holidays')) {
  ...
  return (
    <div className="flex w-full justify-center gap-3  items-center mb-4 text-blue-600">
      <Box
        height={54}
        width={'100%'}
        border={0.5}
...
export default HolidayCalendarImport;
