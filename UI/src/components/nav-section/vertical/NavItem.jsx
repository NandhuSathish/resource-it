...
export function NavItemSub({ item, open = false, active = false, onOpen }) {
  const { path, info, children } = item;
  const { getUserDetails } = useAuth();
  const { role } = getUserDetails();
  let title = item.title;
  if (title === 'Project Requests') {
    item.title = role === 2 ? 'Project Approval' : 'Project Requests';
  }
  if (title === 'Project Requests') {
    title = role === 2 ? 'Project Approval' : 'Project Requests';
  }
  const renderContent = (
    <>
